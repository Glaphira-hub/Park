	...
	Если БаллыКСписанию <> 0 Тогда
		...
		Если УдалитьЦена <> 0 Тогда
			...
			Если Доля > МаксимальнаяДоля Тогда
				Отказ = Истина;
				Сообщение = Новый СообщениеПользователю;
				Сообщение.Текст = СтрШаблон("Доля списываемых баллов от суммы продажи допустимой (%1%%)", МаксимальнаяДоля);
				Сообщение.УстановитьДанные(ЭтотОбъект);
				Сообщение.Поле = "БаллыКСписанию";
				Сообщение.Сообщить();
			КонецЕсли;		
...
Функция ДоляНакапливаемыхБаллов(СуммаПокупокКлиента)
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	ШкалаБонуснойПрограммыДиапозоны.ПроцентНакопления КАК ПроцентНакопления
		|ИЗ
